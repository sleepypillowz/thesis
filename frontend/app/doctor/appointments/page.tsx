...
export default function ReferralsPage() {
  ...
  const fetchReferrals = useCallback(async () => {
    try {
      ...
      setError(err instanceof Error ? err.message : "Error fetching referrals");
    } finally {
      setLoading(false);
    }
      ...
      {selectedReferral && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50">
          <div className="w-full max-w-2xl rounded-xl bg-white p-8 shadow-lg">
            <h3 className="text-2xl font-bold text-gray-900 mb-6">Edit Referral</h3>
            <form onSubmit={handleSubmit}>
                  ...
                  <input
                    ...
                    defaultValue={
                      selectedReferral.appointment_date
                        ? format(parseISO(selectedReferral.appointment_date), "yyyy-MM-dd'T'HH:mm")
                        : ""
                    }
                    className="w-full rounded-lg border border-gray-300 px-4 py-2 focus:border-blue-500 focus:ring-blue-500"
                  />
              ...
              <div className="mt-8 flex justify-end space-x-4">
                <button
                  ...
                  className="rounded-lg px-6 py-2 text-gray-700 hover:bg-gray-100 transition-colors"
                >
                  Cancel
                </button>
                <button
      ...
      {referrals.length === 0 ? (
        ...
        <div className="w-full space-y-6">
          {referrals.map((referral) => {
            ...
            return (
              <div key={referral.id} className="w-full overflow-hidden rounded-xl border border-gray-100 bg-white shadow-sm">
                <div className="p-6">
                  <div className="mb-4 flex flex-wrap items-center justify-between">
                    <div className="flex items-center space-x-3">
                      <h2 className="text-xl font-semibold text-gray-900">{referral.patient}</h2>
